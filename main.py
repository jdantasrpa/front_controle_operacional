...
# Permite apontar outro .ini sem editar o do projeto (homologação, testes).
CONFIG_PATH = Path(
    os.getenv("COFC_CONFIG") or Path(__file__).with_name("config_projeto.ini")
)
...
def auto_refresh_por_aba(numero_convenio, mes_ref, current_page, active_tab):
    ...
    try:
        dados, contatos, parts, contas, msg, _, _ = carregar_convenio_por_chave(
            str(numero_convenio), str(mes_ref)
        )
        ...
        return (
            gr.update(),              # msg
            upd_header,               # header
            upd_state,                # dados_state
            *upd_inputs,              # campos Dados
            df_contatos,              # contatos_df
            df_parts,                 # particularidades_df
            df_contas                 # contas_df
        )
...
def preencher_dados_updates(dados: dict):
    ...
    for f in DADOS_FIELDS:
        ...
        if field_type == "select" and options:
            updates.append(
                gr.update(
                    value=safe_pick_choice_ui(raw, options)
                )
            )
            continue
...
# =========================================================
# Financeiro e Cobrança (banco bd_cobranca_financeiro.db)
#
# Estas duas abas leem de um banco distinto do COFCT: o
# bd_cobranca_financeiro.db do próprio projeto. A LEITURA é direta
# (SELECT); a ESCRITA continua pelo mesmo padrão de fila de comandos
# usado no resto do arquivo, com a chave extra "banco_destino" para o
# writer saber em qual arquivo aplicar o SQL.
# =========================================================

BANCO_DESTINO_COFCT = "cofct"
BANCO_DESTINO_COBRANCA = "cobranca_financeiro"
...
FORMA_PAGAMENTO_OPCOES = ["pix", "ted", "boleto"]

STATUS_COBRANCA_OPCOES = [
    "pendente",
    "em_negociacao",
    "agendado",
    "resolvido",
...
PRIORIDADE_OPCOES = ["baixa", "media", "alta"]

CANAL_COBRANCA_OPCOES = ["telefone", "whatsapp", "email", "presencial"]

RESULTADO_COBRANCA_OPCOES = [
    "sem_resposta",
    "contato_efetivo",
    "retornar",
    "recusado",
...
def get_banco_cobranca() -> Path:
    """Devolve o caminho do banco de cobrança/financeiro do projeto.

    Reaproveita a resolução de caminhos já usada pelo painel web
    (``api/config.py``), evitando duplicar a leitura do .ini.

    Returns:
        Caminho absoluto do ``bd_cobranca_financeiro.db``.

    Raises:
        FileNotFoundError: Se o arquivo não existir.

    Example:
        >>> get_banco_cobranca().name
        'bd_cobranca_financeiro.db'
    """
    from api.config import obter_configuracao

    caminho = obter_configuracao().banco_cobranca
    if not caminho.exists():
        raise FileNotFoundError(f"Banco de cobrança não encontrado: {caminho}")
    return caminho


def db_connect_cobranca():
    """Abre conexão de leitura no banco de cobrança/financeiro.

    Returns:
        Conexão sqlite3 com ``row_factory`` em ``sqlite3.Row``.

    Example:
        >>> conn = db_connect_cobranca()
        >>> conn.close()
    """
    conn = sqlite3.connect(get_banco_cobranca())
    conn.row_factory = sqlite3.Row
    return conn


def _consultar_cobranca(sql: str, params: tuple) -> list[dict]:
    """Executa um SELECT no banco de cobrança e devolve dicionários.

    Args:
        sql: SELECT parametrizado.
        params: Valores dos placeholders.

    Returns:
        Lista de linhas; vazia se o banco estiver indisponível.
    """
    try:
        conn = db_connect_cobranca()
...
def listar_financeiro(numero_convenio: str, mes_ref: str) -> list[dict]:
    """Lista os repasses financeiros do convênio.

    Traz a competência selecionada primeiro e, na sequência, o histórico
    das demais — o operador enxerga o mês em análise sem perder o retrato
    dos meses anteriores.

    Args:
        numero_convenio: Número do convênio.
        mes_ref: Mês de referência em análise.

    Returns:
        Registros de ``tabela_financeiro_repasse``.

    Example:
        >>> listar_financeiro('126225', '2025-07')  # doctest: +SKIP
        [...]
    """
    return _consultar_cobranca(
        """
        SELECT
            id, mes_referencia, data_prevista, data_efetiva,
            valor_previsto, valor_recebido, valor_repassado,
            forma_pagamento, status_financeiro, observacao,
            criado_em, atualizado_em
        FROM tabela_financeiro_repasse
        WHERE numero_convenio = ?
        ORDER BY (mes_referencia = ?) DESC, mes_referencia DESC, id DESC
        """,
        (str(numero_convenio).strip(), str(mes_ref).strip()),
    )


def listar_cobranca_casos(numero_convenio: str, mes_ref: str) -> list[dict]:
    """Lista os casos de cobrança abertos para o convênio.

    Args:
        numero_convenio: Número do convênio.
        mes_ref: Mês de referência em análise (ordena para o topo).

    Returns:
        Registros de ``tabela_cobranca_caso``.

    Example:
        >>> listar_cobranca_casos('126225', '2025-07')  # doctest: +SKIP
        [...]
    """
    return _consultar_cobranca(
        """
        SELECT
            id, mes_referencia, valor_em_aberto, status_cobranca,
            motivo, prioridade, contato_nome, responsavel, observacao,
            criado_em, atualizado_em
...
def listar_cobranca_historico(numero_convenio: str) -> list[dict]:
    """Lista o histórico de tentativas de cobrança do convênio.

    Args:
        numero_convenio: Número do convênio.

    Returns:
        Tentativas de ``tabela_cobranca_tentativa``, da mais recente
        para a mais antiga.

    Example:
        >>> listar_cobranca_historico('126225')  # doctest: +SKIP
        [...]
    """
    return _consultar_cobranca(
        """
        SELECT
            t.id, t.data_hora, t.canal, t.resultado, t.contato_nome,
            t.observacao, t.ator, t.criado_em
        FROM tabela_cobranca_tentativa AS t
        JOIN tabela_cobranca_caso AS c ON c.id = t.id_caso
        WHERE c.numero_convenio = ?
        ORDER BY t.data_hora DESC, t.id DESC
        """,
        (str(numero_convenio).strip(),),
    )


def listar_contatos_ativos(numero_convenio: str) -> list[str]:
    """Devolve os contatos ATIVOS do convênio para o registro de cobrança.

    O registro de cobrança é feito "com base no contato": o operador
    escolhe quem foi acionado a partir do cadastro real do convênio.

    Args:
        numero_convenio: Número do convênio.

    Returns:
        Rótulos ``Nome — Área — Telefone`` prontos para o dropdown.

    Example:
        >>> listar_contatos_ativos('126225')  # doctest: +SKIP
        ['Paulo Souza — Jurídico — (11) 91641-5059']
    """
    numero_convenio = (str(numero_convenio) if numero_convenio else "").strip()
...
def nome_do_contato(rotulo: str) -> str:
    """Extrai o nome puro do rótulo montado por :func:`listar_contatos_ativos`.

    Args:
        rotulo: Texto no formato ``Nome — Área — Telefone``.

    Returns:
        Apenas o nome do contato.

    Example:
        >>> nome_do_contato('Paulo Souza — Jurídico — (11) 9')
        'Paulo Souza'
    """
    return (rotulo or "").split(" — ")[0].strip()


def _enfileirar(metodo: str, sql: str, params: list, banco_destino: str) -> Path:
    """Grava um comando na fila do writer.

    Args:
        metodo: Identificador da operação (ex.: ``INSERT_COBRANCA_CASO``).
        sql: Comando SQL parametrizado.
        params: Valores dos placeholders.
        banco_destino: Qual banco o writer deve abrir para aplicar.

    Returns:
        Caminho do arquivo de comando gerado.
    """
    payload = {
        "id": _make_cmd_id(),
        "created_at": _created_at_br(),
        "actor": get_actor(),
        "bd_utilizado": get_latest_db_path().name,
        "banco_destino": banco_destino,
        "metodo": metodo,
        "sql": " ".join(sql.split()),
        "params": params,
    }
    return _queue_write(PASTA_ENTRADA, payload)
...
def salvar_financeiro(
    ...
    """Enfileira a criação/edição de um repasse financeiro do convênio.

    Args:
        numero_convenio: Número do convênio selecionado.
        mes_ref: Mês de referência em análise (valida o convênio).
        financeiro_id: Id do repasse ao editar; vazio ao criar.
        mes_referencia: Competência do repasse.
        data_prevista: Data prevista de recebimento.
        data_efetiva: Data efetiva de recebimento.
        valor_previsto: Valor previsto (aceita formato BR).
        valor_recebido: Valor recebido (aceita formato BR).
        valor_repassado: Valor repassado (aceita formato BR).
        forma_pagamento: pix | ted | boleto.
        status_financeiro: Situação do repasse.
        observacao: Texto livre.

    Returns:
        Tupla ``(mensagem_html, lista_de_repasses)``.

    Example:
        >>> salvar_financeiro('1', '2025-07', None, '2025-07', ...)  # doctest: +SKIP
    """
    if not numero_convenio or not mes_ref:
        return (
            "<div class='alert'>Selecione um convênio e o mês referência.</div>",
            [],
        )

    conv = _validar_convenio(numero_convenio, mes_ref)
    if conv is None:
        return (
            "<div class='alert error'>Convênio não encontrado para este mês.</div>",
            listar_financeiro(numero_convenio, mes_ref),
        )

    competencia = (mes_referencia or "").strip() or str(mes_ref).strip()
    forma = normalize_choice(
        forma_pagamento, FORMA_PAGAMENTO_OPCOES, default="pix", field="Forma de pagamento"
    )
    situacao = normalize_choice(
        status_financeiro,
        STATUS_FINANCEIRO_OPCOES,
        default="previsto",
        field="Status financeiro",
    )
    now_sql = _now_sql()

    id_norm = None
    try:
        if financeiro_id not in (None, "", 0, "0"):
            id_norm = int(financeiro_id)
    except (TypeError, ValueError):
        id_norm = None

    if id_norm:
        metodo = "UPDATE_FINANCEIRO_REPASSE"
        sql = """
            UPDATE tabela_financeiro_repasse
               SET mes_referencia = ?, data_prevista = ?, data_efetiva = ?,
                   valor_previsto = ?, valor_recebido = ?, valor_repassado = ?,
                   forma_pagamento = ?, status_financeiro = ?, observacao = ?,
                   atualizado_em = ?
             WHERE id = ?
        ...
        params = [
            competencia,
            (data_prevista or "").strip(),
            (data_efetiva or "").strip(),
            parse_br_number(valor_previsto),
            parse_br_number(valor_recebido),
            parse_br_number(valor_repassado),
            forma,
            situacao,
            (observacao or "").strip(),
            now_sql,
        ...
        metodo = "INSERT_FINANCEIRO_REPASSE"
        sql = """
            INSERT OR IGNORE INTO tabela_financeiro_repasse
                (originador, numero_convenio, nome_convenio, cnpj_convenio,
                 mes_referencia, data_prevista, data_efetiva,
                 valor_previsto, valor_recebido, valor_repassado,
                 forma_pagamento, status_financeiro, observacao,
                 criado_em, atualizado_em)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    arq = _enfileirar(metodo, sql, params, BANCO_DESTINO_COBRANCA)

    return (
        f"<div class='alert success'>Comando enfileirado: {metodo} ({arq.name})</div>",
        listar_financeiro(numero_convenio, mes_ref),
    )


def _validar_convenio(numero_convenio, mes_ref):
    """Carrega a chave do convênio no snapshot COFCT.

    Args:
        numero_convenio: Número do convênio.
        mes_ref: Mês de referência da conciliação.

    Returns:
        Linha com originador/numero/nome/cnpj, ou ``None`` se não existir.
    """
    try:
        conn = db_connect()
    except Exception:
    ...
    try:
        return conn.execute(
            """
            SELECT originador, numero_convenio, nome_convenio, cnpj_convenio
            FROM tabela_concilicacao_convenio
            WHERE numero_convenio = ?
              AND mes_referencia_conciliacao = ?
            LIMIT 1
...
def salvar_cobranca(
    ...
    """Enfileira um registro de cobrança feito a partir de um contato.

    Gera dois comandos encadeados: o caso da competência (idempotente,
    graças à UNIQUE ``originador+numero_convenio+mes_referencia``) e a
    tentativa de contato. A tentativa resolve o ``id_caso`` por subquery,
    pois na fila o id ainda não existe no momento da gravação.

    Args:
        numero_convenio: Número do convênio selecionado.
        mes_ref: Mês de referência em análise.
        contato_rotulo: Contato acionado (vindo do cadastro do convênio).
        canal: telefone | whatsapp | email | presencial.
        resultado: Desfecho da tentativa.
        valor_em_aberto: Valor da pendência (aceita formato BR).
        status_cobranca: Situação do caso.
        prioridade: baixa | media | alta.
        observacao: O que foi tratado.
        data_hora: Data/hora da tentativa.

    Returns:
        Tupla ``(mensagem_html, casos, historico)``.

    Example:
        >>> salvar_cobranca('1', '2025-07', 'Ana', 'telefone', ...)  # doctest: +SKIP
    """
    if not numero_convenio or not mes_ref:
        return (
            "<div class='alert'>Selecione um convênio e o mês referência.</div>",
    ...
    contato = nome_do_contato(contato_rotulo)
    if not contato:
        return (
            "<div class='alert error'>Selecione o contato acionado (aba Contato).</div>",
            listar_cobranca_casos(numero_convenio, mes_ref),
            listar_cobranca_historico(numero_convenio),
        )
    ...
    canal_ok = normalize_choice(
        canal, CANAL_COBRANCA_OPCOES, default="telefone", field="Canal"
    )
    resultado_ok = normalize_choice(
        resultado, RESULTADO_COBRANCA_OPCOES, default="sem_resposta", field="Resultado"
    )
    status_ok = normalize_choice(
        status_cobranca, STATUS_COBRANCA_OPCOES, default="pendente", field="Status"
    )
    prioridade_ok = normalize_choice(
        prioridade, PRIORIDADE_OPCOES, default="media", field="Prioridade"
    )

    now_sql = _now_sql()
    quando = (data_hora or "").strip() or now_sql
    actor = get_actor()

    # 1) Caso da competência — não duplica graças ao OR IGNORE + UNIQUE.
    sql_caso = """
        INSERT OR IGNORE INTO tabela_cobranca_caso
            (originador, numero_convenio, nome_convenio, cnpj_convenio,
             mes_referencia, valor_em_aberto, status_cobranca, motivo,
             prioridade, responsavel, observacao, criado_em, atualizado_em)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """
    params_caso = [
        conv["originador"],
        ...
        conv["cnpj_convenio"],
        str(mes_ref).strip(),
        parse_br_number(valor_em_aberto),
        status_ok,
        "controle_analitico",
        prioridade_ok,
        actor,
    ...
    arq_caso = _enfileirar(
        "INSERT_COBRANCA_CASO", sql_caso, params_caso, BANCO_DESTINO_COBRANCA
    )

    # 2) Atualiza o caso já existente (status/valor/prioridade da vez).
    sql_status = """
        UPDATE tabela_cobranca_caso
           SET status_cobranca = ?, valor_em_aberto = ?, prioridade = ?,
               atualizado_em = ?
         WHERE originador = ?
           AND numero_convenio = ?
           AND mes_referencia = ?
    ...
    _enfileirar(
        "UPDATE_COBRANCA_CASO", sql_status, params_status, BANCO_DESTINO_COBRANCA
    )

    # 3) Tentativa — o id_caso é resolvido no momento em que o writer aplica.
    sql_tentativa = """
    ...
    arq_tent = _enfileirar(
        "INSERT_COBRANCA_TENTATIVA",
        sql_tentativa,
        params_tentativa,
        BANCO_DESTINO_COBRANCA,
    )

    msg = (
        "<div class='alert success'>Cobrança registrada para "
        f"<b>{contato}</b> — 3 comandos enfileirados "
        f"({arq_caso.name}, {arq_tent.name}).</div>"
    )
...
def build_app():
    ...
    */
    ...
    # Competência e originadora são obrigatórias no Sintético, então
    # entram sem a opção "Todos" que get_filter_options() prefixa.
    MESES_DISPONIVEIS = [m for m in meses if m != "Todos"]
    ORIGINADORAS_DISPONIVEIS = [o for o in originadores if o != "Todos"]

    titulo_sistema = "SCO - Controle Operacional"
    ...
    with gr.Blocks(
        ...
        with gr.Column(visible=False) as page_sintetico:
            with gr.Column(elem_classes=["card-controle_sintetico", "card"]):
                ...
                gr.Markdown(
                    "<div class='muted'>O controle é <b>mensal por originadora</b>: "
                    "escolha a competência e a originadora para listar os convênios.</div>"
                )
                    ...
                    with gr.Column(scale=10, min_width=400):
                        with gr.Accordion("Filtros", open=True):
                            with gr.Row():
                                ...
                                # Competência e originadora são obrigatórias e
                                # saem do próprio snapshot — nada de digitar um
                                # mês que não existe na base.
                                f_mes = gr.Dropdown(
                                    elem_classes=["txt-limitado"],
                                    choices=MESES_DISPONIVEIS,
                                    value=None,
                                    label="Mês (referência) *",
                                )
                                f_originador = gr.Dropdown(
                                    elem_classes=["txt-limitado"],
                                    choices=ORIGINADORAS_DISPONIVEIS,
                                    value=None,
                                    label="Originadora *",
                                )
        ...
        with gr.Column(visible=False) as page_analitico:
            with gr.Column(elem_classes=["card-controle_analitico", "card"]):
                ...
                with gr.Tabs() as tabs:
                    with gr.Tab("Conciliação", elem_classes=['aba_style']):
                        with gr.Column(elem_classes=["card-controle_analitico_abas", "card"]):
                            gr.HTML("<div class='aba_text_fora_campo'>Conciliação do Convênio (edição controlada por campo)</div>")
                    ...
                    with gr.Tab("Financeiro"):
                        with gr.Column(elem_classes=["card-controle_analitico_abas", "card"]):
                            gr.HTML(
                                "<div class='aba_text_fora_campo'>Repasses do Convênio "
                                "(previsto x recebido x repassado)</div>"
                            )
                            financeiro_df = gr.Dataframe(
                                value=ensure_df([], FINANCEIRO_COLS),
                                headers=FINANCEIRO_COLS,
                                datatype=make_datatypes(FINANCEIRO_COLS),
                                interactive=False,
                                wrap=True,
                                row_count=8,
                                column_count=(len(FINANCEIRO_COLS), "fixed"),
                                label=""
                            )

                        with gr.Column(elem_classes=["card"]):
                            gr.HTML("<div class='aba_text_fora_campo'>Formulário de repasse (criar/editar)</div>")
                            financeiro_id = gr.State(None)

                            with gr.Row():
                                fin_mes_referencia = gr.Textbox(label="Competência", placeholder="AAAA-MM")
                                fin_status = gr.Dropdown(
                                    label="Status financeiro",
                                    choices=STATUS_FINANCEIRO_OPCOES,
                                    value="previsto",
                                )
                            with gr.Row():
                                fin_data_prevista = gr.Textbox(label="Data prevista", placeholder="AAAA-MM-DD")
                                fin_data_efetiva = gr.Textbox(label="Data efetiva", placeholder="AAAA-MM-DD")
                            with gr.Row():
                                fin_valor_previsto = gr.Textbox(label="Valor previsto")
                                fin_valor_recebido = gr.Textbox(label="Valor recebido")
                            with gr.Row():
                                fin_valor_repassado = gr.Textbox(label="Valor repassado")
                                fin_forma_pagamento = gr.Dropdown(
                                    label="Forma de pagamento",
                                    choices=FORMA_PAGAMENTO_OPCOES,
                                    value="pix",
                                )
                            fin_observacao = gr.Textbox(label="Observação")

                            with gr.Row():
                                btn_salvar_financeiro = gr.Button("Salvar repasse", variant="primary")
                                btn_novo_financeiro = gr.Button("Novo / Limpar", variant="secondary")

                            msg_financeiro = gr.HTML("")
                    ...
                    with gr.Tab("Cobrança"):
                        # (a) Histórico de cobrança
                        with gr.Column(elem_classes=["card-controle_analitico_abas", "card"]):
                            gr.HTML("<div class='aba_text_fora_campo'>Casos de cobrança do Convênio</div>")
                            cobranca_casos_df = gr.Dataframe(
                                value=ensure_df([], COBRANCA_CASO_COLS),
                                headers=COBRANCA_CASO_COLS,
                                datatype=make_datatypes(COBRANCA_CASO_COLS),
                                interactive=False,
                                wrap=True,
                                row_count=5,
                                column_count=(len(COBRANCA_CASO_COLS), "fixed"),
                                label=""
                            )

                        with gr.Column(elem_classes=["card-controle_analitico_abas", "card"]):
                            gr.HTML("<div class='aba_text_fora_campo'>Histórico de cobrança (tentativas de contato)</div>")
                            cobranca_hist_df = gr.Dataframe(
                                value=ensure_df([], COBRANCA_HISTORICO_COLS),
                                headers=COBRANCA_HISTORICO_COLS,
                                datatype=make_datatypes(COBRANCA_HISTORICO_COLS),
                                interactive=False,
                                wrap=True,
                                row_count=8,
                                column_count=(len(COBRANCA_HISTORICO_COLS), "fixed"),
                                label=""
                            )

                        # (b) Registro de cobrança com base no contato
                        with gr.Column(elem_classes=["card"]):
                            gr.HTML(
                                "<div class='aba_text_fora_campo'>Registrar cobrança "
                                "(o contato vem do cadastro da aba Contato)</div>"
                            )

                            with gr.Row():
                                cob_contato = gr.Dropdown(
                                    label="Contato acionado *",
                                    choices=[],
                                    value=None,
                                    interactive=True,
                                )
                                cob_data_hora = gr.Textbox(
                                    label="Data/hora", placeholder="AAAA-MM-DD HH:MM:SS"
                                )
                            with gr.Row():
                                cob_canal = gr.Dropdown(
                                    label="Canal",
                                    choices=CANAL_COBRANCA_OPCOES,
                                    value="telefone",
                                )
                                cob_resultado = gr.Dropdown(
                                    label="Resultado",
                                    choices=RESULTADO_COBRANCA_OPCOES,
                                    value="sem_resposta",
                                )
                            with gr.Row():
                                cob_valor_aberto = gr.Textbox(label="Valor em aberto")
                                cob_status = gr.Dropdown(
                                    label="Status da cobrança",
                                    choices=STATUS_COBRANCA_OPCOES,
                                    value="pendente",
                                )
                            with gr.Row():
                                cob_prioridade = gr.Dropdown(
                                    label="Prioridade",
                                    choices=PRIORIDADE_OPCOES,
                                    value="media",
                                )
                                cob_observacao = gr.Textbox(label="Observação")

                            with gr.Row():
                                btn_salvar_cobranca = gr.Button("Registrar cobrança", variant="primary")
                                btn_novo_cobranca = gr.Button("Novo / Limpar", variant="secondary")

                            msg_cobranca = gr.HTML("")
        ...
        # -------------------------
        def _tabela_vazia():
            """Devolve o par (view, tech) vazio, no schema esperado."""
            df_tech = pd.DataFrame(columns=SINT_COLS_TECH)
            df_view = df_tech[SINT_COLS_VIEW].rename(columns=SINT_LABELS_VIEW)
            return df_view, df_tech

        def do_filtrar(mes, status, motivo, originador, data_vencimento, data_sla, data_corte, busca):
            # Controle mensal por originadora: sem os dois, não há listagem.
            faltando = [
                rotulo
                for rotulo, valor in (("Mês (referência)", mes), ("Originadora", originador))
                if not (valor or "").strip()
            ]
            if faltando:
                df_view, df_tech = _tabela_vazia()
                return (
                    "<div class='alert-invert'>Selecione "
                    f"<b>{' e '.join(faltando)}</b> para listar os convênios.</div>",
                    df_view,
                    df_tech,
                )
            ...
            return (
                f"<div class='alert-invert'>{originador} | {mes} — {info}</div>",
                df_view,
                df_tech,
            )

        def do_limpar():
            return None, "Todos", "Todos", None, "", "", "", ""
        ...
        def abrir_analitico(numero_convenio, mes_ref):
            ...
            # lista de updates para os inputs do form "Dados"
            clear_updates = [gr.update(value="") for _ in DADOS_FIELDS]
            ...
            if not numero_convenio or not mes_ref:
                return (
                    ...
                    *clear_updates,
                    ...
                    # dataframes (nunca string!)
                    ensure_df([], CONTATO_COLS),
                    ensure_df([], PART_COLS),
                    ensure_df([], CONTA_COLS),
                    ensure_df([], FINANCEIRO_COLS),
                    ensure_df([], COBRANCA_CASO_COLS),
                    ensure_df([], COBRANCA_HISTORICO_COLS),
            ...
            # Abas Financeiro e Cobrança vêm do bd_cobranca_financeiro.db.
            financeiro = listar_financeiro(numero_convenio, mes_ref)
            casos = listar_cobranca_casos(numero_convenio, mes_ref)
            historico = listar_cobranca_historico(numero_convenio)
            contatos_choices = listar_contatos_ativos(numero_convenio)
            ...
            return (
                ...
                *updates,
                ...
                ensure_df(parts, PART_COLS),
                ensure_df(contas, CONTA_COLS),
                ensure_df(financeiro, FINANCEIRO_COLS),
                ensure_df(casos, COBRANCA_CASO_COLS),
                ensure_df(historico, COBRANCA_HISTORICO_COLS),
                gr.update(
                    choices=contatos_choices,
                    value=contatos_choices[0] if contatos_choices else None,
                ),
            )
        ...
        def abrir_por_duplo_clique(flag, numero_convenio, mes_ref, current_page):
            ...
            # abrir_analitico retorna MAIS (inclui selected_numero_convenio e selected_mes_ref)
            ret = abrir_analitico(numero_convenio, mes_ref)

            # ret layout (pelo seu botão Abrir):
            # 0..6 nav (7)
            # 7..8 states numero/mes (2)
            # 9 header
            # 10 dados_state
            # 11..(11+len(DADOS_FIELDS)-1) inputs
            # o restante são os componentes das abas (dataframes + dropdown)

            # Para o change do abrir_flag, queremos SEM os 2 states (numero/mes)
            nav = ret[:7]
            header = ret[9]
            dados = ret[10]
            inputs = ret[11:11+len(DADOS_FIELDS)]
            dfs = ret[11+len(DADOS_FIELDS):]  # componentes das abas

            return (*nav, header, dados, *inputs, *dfs)
        ...
        btn_ir_analitico.click(
            ...
            outputs=[
                ...
                *DADOS_OUTPUTS,
                ...
                financeiro_df, cobranca_casos_df, cobranca_hist_df, cob_contato
            ]
        )
        ...
        # Load inicial
        # -------------------------
        # Sem competência/originadora escolhidas não há o que listar: o load
        # inicial apenas orienta o operador.
        demo.load(
            fn=lambda: do_filtrar("", "Todos", "Todos", "", "", "", "", ""),
            inputs=[],
            outputs=[resumo, tabela, tabela_tech]
        )
        # =========================
        ...
        # =========================
        # CRUD / Select - Financeiro
        # =========================
        def limpar_form_financeiro():
            """Zera o formulário de repasse (novo registro)."""
            return None, "", "", "", "", "", "", "pix", "previsto", ""

        def on_select_financeiro(df, evt: gr.SelectData):
            """Carrega no formulário o repasse clicado na tabela."""
            try:
                r = df.iloc[evt.index[0]].to_dict()
            except Exception:
                return limpar_form_financeiro()

            return (
                r.get("id"),
                r.get("mes_referencia", "") or "",
                r.get("data_prevista", "") or "",
                r.get("data_efetiva", "") or "",
                br_money(r.get("valor_previsto")),
                br_money(r.get("valor_recebido")),
                br_money(r.get("valor_repassado")),
                _coerce_choice(r.get("forma_pagamento"), FORMA_PAGAMENTO_OPCOES, "pix"),
                _coerce_choice(
                    r.get("status_financeiro"), STATUS_FINANCEIRO_OPCOES, "previsto"
                ),
                r.get("observacao", "") or "",
            )
        ...
        financeiro_df.select(
            fn=on_select_financeiro,
            inputs=[financeiro_df],
            outputs=FINANCEIRO_FORM,
        )

        btn_novo_financeiro.click(
            fn=limpar_form_financeiro,
            inputs=[],
            outputs=FINANCEIRO_FORM,
        )

        def salvar_financeiro_e_recarregar(
            numero_convenio,
            mes_ref,
            financeiro_id_val,
            mes_referencia,
            ...
            """Enfileira o repasse e devolve a lista atualizada."""
            msg, linhas = salvar_financeiro(
                numero_convenio,
                mes_ref,
                financeiro_id_val,
                mes_referencia,
            ...
            return msg, ensure_df(linhas, FINANCEIRO_COLS)

        btn_salvar_financeiro.click(
            fn=salvar_financeiro_e_recarregar,
            inputs=[selected_numero_convenio, selected_mes_ref, *FINANCEIRO_FORM],
            outputs=[msg_financeiro, financeiro_df],
        )

        # =========================
        # CRUD - Cobrança
        # =========================
        def limpar_form_cobranca():
            """Zera o formulário de registro de cobrança."""
            return "", "telefone", "sem_resposta", "", "pendente", "media", ""

        def salvar_cobranca_e_recarregar(
            numero_convenio,
            mes_ref,
            contato_rotulo,
            data_hora,
            ...
            """Registra a cobrança e devolve casos e histórico atualizados."""
            msg, casos, historico = salvar_cobranca(
                numero_convenio,
                mes_ref,
                contato_rotulo,
                canal,
        ...
        btn_salvar_cobranca.click(
            fn=salvar_cobranca_e_recarregar,
            inputs=[
                selected_numero_convenio,
                selected_mes_ref,
                cob_contato,
            ...
            outputs=[msg_cobranca, cobranca_casos_df, cobranca_hist_df],
        )

        btn_novo_cobranca.click(
            fn=limpar_form_cobranca,
            inputs=[],
            outputs=[
                cob_data_hora,
                cob_canal,
